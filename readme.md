...
*   **Advanced Authentication & Control:**
    ...
    *   **True Plex Watchlist Integration:** The `Plex to Trakt` watchlist sync now uses your **native, universal Plex Watchlist** as the source of truth, eliminating the need for manual collection management and providing a seamless, intuitive experience.
...
*   **Comprehensive Collection & List Management:**
    *   **Plex Library to Trakt Collection Sync:** A one-way sync that populates your Trakt "Collection" (items you own) with the entire contents of your selected Plex libraries.
    *   **Custom Trakt Lists to Plex Sync:** A powerful one-way feature that fetches your custom lists from Trakt and automatically creates corresponding, synchronized collections on your Plex server.
    *   **Destructive Clear Collection Tool:** For users wanting a clean slate, an optional setting allows you to **completely wipe your Trakt Collection** before a sync runs, ensuring it becomes a perfect mirror of your Plex library.
...
*   **Backend Stability & Performance:**
    ...
    *   **Intelligent Sync Safeguards:** To prevent data corruption, an **intelligent safety fuse** is built into the sync logic. It automatically aborts any sync that would add an abnormally large number of items to an *established* Trakt account, protecting your data from bugs. This safeguard is also smart enough to **automatically disengage for an initial sync to a new or empty account**, allowing for safe and seamless population of your history.
...
*   **Plex Integration:** Utilizes the `PlexAPI` library for stable and reliable communication with the user's Plex Media Server, including direct access to the account-level universal watchlist.
*   **Trakt Integration (A Hybrid & Optimized Approach):**
    *   The core synchronization engine leverages direct, authenticated API calls using the `requests` library for maximum speed, data richness, and control, bypassing potential library limitations.
...
4.  **Run the Application:**
    ...
    python app.py
    ```
    The application features a robust launcher that automatically detects and utilizes the correct Python virtual environment. After starting, proceed to the "Usage" section below. If you encounter API errors during authorization, see the Troubleshooting section.
...
1.  **First-Time Setup:**
    ...
    *   Configure your desired sync features and direction using the checkboxes.
    *   Click **Save Settings**.
    *   Re-open Settings, select the Plex libraries you wish to sync.
...
<details>
<summary><strong>Experiencing API errors? Click here for instructions on patching the `trakt` library.</strong></summary>

In some environments, the `trakt` library (version 3.4.0) installed via `pip` can have issues with its API endpoint configuration, leading to authentication or sync errors. If you experience these problems, a simple, one-time manual patch to the library files within your virtual environment will resolve them permanently.

**Step 1: Locate the Library Files**

After installing dependencies with `pip install -r requirements.txt`, navigate to the `trakt` library's folder inside your virtual environment:

*   **Windows:** `venv\Lib\site-packages\trakt\`
*   **macOS/Linux:** `venv/lib/pythonX.X/site-packages/trakt/` (the Python version may vary)

You will need to edit two files: `core.py` and `users.py`.

**Step 2: Patch `core.py`**

This patch corrects an outdated API URL.

1.  Open the file: `trakt/core.py`
2.  Find this line:
    ```python
    BASE_URL = 'https://api-v2launch.trakt.tv/'
    ```
3.  Replace it with the correct URL:
    ```python
    BASE_URL = 'https://api.trakt.tv/'
    ```

**Step 3: Patch `users.py`**

This patch corrects how the library builds API request URLs.

1.  Open the file: `trakt/users.py`
2.  At the top of the file, add these two import statements if they are not already present:
    ```python
    from urllib.parse import urljoin
    import trakt.core
    ```
3.  Find the `_get` method inside the `User` class and **replace the entire method** with the following code:
    ```python
    def _get(self, path, **kwargs):
        if self.username == 'me':
            # Use a fixed path for the current user
            final_path = 'users/me'
        else:
            # Construct the path correctly for other users
            final_path = f'users/{self.username}'

        # Combine the base URL with the final path
        url = urljoin(trakt.core.BASE_URL, final_path)
        
        # Add the original path back if it's not the base user path
        if path:
            url = urljoin(f"{url}/", path)

        with self.client.session.get(url, **kwargs) as response:
            return self.client.handle_response(response)
    ```

Once these two files are patched, the `trakt` library will function correctly. You can now run `python app.py` without API issues.
